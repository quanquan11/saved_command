...
function openSavePopup() {
    const command = document.getElementById('command-input').value;
    const category_id = 3;  // Assuming 3 is for FDAT95
    if (command) {
...
function showAllCommands() {
    ...
    fetch(`/get_saved_commands?category_id=3`)  // FDAT95 category_id = 3
        .then(response => response.json())
        .then(data => {
            if (data.length > 0) {
...
function searchCommands() {
    ...
    fetch(`/get_saved_commands?category_id=3`)  // category_id=3 is for FDAT95
        .then(response => response.json())
        .then(data => {
